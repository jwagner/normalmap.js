...
function copyAssets(release, dest){
    ...
    return streamsToPromise([
        copyImages(['demos/gfx/*/{ambient,baseColor,material}.png'], dest, release && 'quantize'),
        ...
        return streamToPromise(
            gulp.src([dest + '?(jquery.)normalmap.js']).pipe(gulp.dest(dest + 'demos/'))
        );
    });
}
...
function buildHtml(release, dest){
    ...
    var stream = gulp.src(['demos/*.jade', '!demos/_*.jade'])
        .pipe(data(function(file){
            return {
                meta: meta,
                release: release,
                baseName: gutil.replaceExtension(path.basename(file.path), '.html').replace('index.html', ''),
...
gulp.task('buildHtml', ['copyAssets'], buildHtml.bind(this, false, BUILD_DIR_DEV + 'demos'));
...
gulp.task('release', ['buildRelease'], function(done){
    rsync({
...
gulp.task('zip', function(){
    ...
    return build(false, BUILD_DIR_DEV, true)
        .then(function(){
            ...
            glob(BUILD_DIR_DEV + '**/*', globOptions, function(e, files){
                ...
                files.forEach(function(file){
                    ...
                    console.log(file);
                    zipFile.addFile(file, file.replace(BUILD_DIR_DEV, meta.name + '/'));
                });
                zipFile.end();
            });
            ...
            var stream = zipFile.outputStream
                .pipe(fs.createWriteStream('build/zip/' + name + '.zip'));
